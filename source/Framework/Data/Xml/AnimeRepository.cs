    ...
    public class AnimeRepository : Repository<Anime>
    {
        ...
        internal override Anime ToEntity(XElement item)
        {
            ...
            temp = new Anime()
            {
                ...
                Category = (ANIME_TYPE)Expression.IfIntegerNull(item.Attribute("category").Value, 1),
                State = (EntityState)Expression.IfIntegerNull(item.Element("state").Value, 0),
                Comment = item.Element("comment").Value
            };
            temp.Codigo = Expression.IfIntegerNull(item.Attribute("id").Value, 0);
